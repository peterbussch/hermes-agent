...
class DDGSWebSearchProvider(BaseWebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("DDGS search timed out after %ds for query_sha256=%s", _SEARCH_TIMEOUT_SECS, query_sha256[:12])
            return search_fail(
            ...
            logger.info("DDGS search interrupted for query_sha256=%s", query_sha256[:12])
            return search_fail("DuckDuckGo search interrupted")
        except Exception as exc:  # noqa: BLE001 — ddgs raises its own exceptions
            logger.warning("DDGS search error: %s", exc)
            return search_fail(f"DuckDuckGo search failed: {exc}")
        logger.info("DDGS search query_sha256=%s: %d results (limit %d)", query_sha256[:12], len(web_results), limit)
