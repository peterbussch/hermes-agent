...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    # Fork: a provider block with its own base_url is a gateway, so vendor/model slugs are legitimate there.
    user_providers = cfg.get("providers")
    user_provider = next(
        (
            entry
    ...
    # Upstream v0.21.4: openai-api pointed at a non-OpenAI endpoint (local router, proxy) is an aggregator in all
    # but name — the router owns the model namespace, so vendor/model slugs are the correct IDs there.
