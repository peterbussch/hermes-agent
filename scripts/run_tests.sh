...
#   * Proper venv activation (probes .venv, venv, then ~/Code/hermes-agent/venv)
#
# Usage:
...
# may have bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/Code/hermes-agent/venv"; do
