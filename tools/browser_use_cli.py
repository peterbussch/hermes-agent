...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        if get_browser_backend() or _camofox_active():
            return None
        cmd = _find_cli()
        ...
        return ("Browser Use is installed but not ready, so Hermes is using the built-in browser tools: "
                f"{readiness_error}. Run `browser-use --doctor` after starting Chrome, or choose another backend in `hermes tools`.")
...
def _browser_use_readiness(cmd: List[str], *, refresh: bool = False) -> Tuple[bool, Optional[str]]:
    """Return operational readiness based on required ``browser-use --doctor`` failures."""
    key = tuple(cmd)
    ...
    try:
        proc = subprocess.run(
            [*cmd, "--doctor"], capture_output=True, text=True, encoding="utf-8",
        ...
        if required_failures:
            ...
            result = (False, details[-1] if details else f"Browser Use doctor exited {proc.returncode}")
        else:
            result = (True, None)
