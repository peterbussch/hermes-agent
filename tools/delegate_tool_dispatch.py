...
def _execute_and_aggregate(batch: _Batch, *, honor_parent_interrupt: bool = True) -> dict:
    ...
    from tools.delegation_live_log import live_run_pointers, update_manifest_statuses
    results: list = []
    ...
    live_pointers = live_run_pointers(batch.live_deleg_id)
...
def _dispatched_payload(batch: _Batch, units: List[tuple[_Batch, str]]) -> dict:
    ...
    from tools.delegation_live_log import live_run_pointers
    payload.update(live_run_pointers(batch.live_deleg_id))
    return payload
