...
def _validate_child_output_schema(
    ...
    if isinstance(_retry_result, dict):
        ...
        for terminal_key in (
            "completed", "interrupted", "failed", "turn_exit_reason", "guardrail", "error",
        ):
...
def _build_result_entry(
    ...
    schema_invalid_after_retry = bool(schema.schema is not None and schema.retries and schema.valid is False)
    if result.get("interrupted", False):
        ...
        status, exit_reason = "failed", "schema_validation_failed"
    ...
    if status == "failed":
        if guardrail_halted:
            entry["error"] = result.get("error", "Subagent halted by a tool-call guardrail.")
        elif schema_invalid_after_retry:
            entry["error"] = result.get("error", "Subagent output did not satisfy output_schema after one retry.")
