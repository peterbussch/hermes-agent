...
async def _dispatch_extract(
    ...
    *,
    ...
    try:
        ...
        if not rescue or not _rescue_eligible(provider):
            raise
        failed = [_result_entry(u, str(exc)) for u in fetch_urls]
        return await asyncio.to_thread(_rescue_extract, provider.name, fetch_urls, failed)
    if rescue and results and all(r.get("error") for r in results) and _rescue_eligible(provider):
    ...
    if cache_successes:
        # Cache under the REQUESTED url each result reports as its own — never by list position:
        # providers omit failed URLs or return successes out of request order, and a positional write
        # filed one page's text under another URL's key for the whole TTL. ``metadata.sourceURL`` counts
        # because Keenable/Firecrawl put the requested URL there when ``url`` is the redirect target. An
        # entry naming no requested URL is served but not cached (a miss re-fetches; a mis-key poisons).
    ...
    *,
    ...
    for position, url in enumerate(safe_urls):
        try:
            ...
            _policy_block = None
        hit = (
            extract_cache_get(url, format=format, provider=provider.name)
            if use_cache and _policy_block is None
            else None
        )
        if hit is not None:
    ...
    results = await _dispatch_extract(
        provider,
        fetch_urls,
        format,
        rescue=rescue,
        cache_successes=use_cache,
    )
