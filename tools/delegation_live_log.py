...
def _live_artifact(path: str, task_index: int) -> Dict[str, Any]:
    ...
    with artifact_path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    return {
        "type": "live_transcript", "task_index": task_index, "path": str(artifact_path),
        "sha256": digest.hexdigest(), "size_bytes": artifact_path.stat().st_size,
    }
...
def _live_artifacts(paths: List[str]) -> List[Dict[str, Any]]:
    ...
    for task_index, path in enumerate(paths):
        with _best_effort(f"artifact hash {path}"):
...
def _write_manifest(delegation_id: str, task_list: List[Dict[str, Any]],
    ...
    with _best_effort("manifest write"):
        ...
        _dump_json(_manifest_path(delegation_id), {
            "schema_version": 1, **pointers, "started": time.strftime(_TIME_FMT),
...
def update_manifest_statuses(delegation_id: Optional[str],
    ...
    with _best_effort("manifest update"):
        ...
        manifest["artifacts"] = _live_artifacts([
            task["log"] for task in manifest.get("tasks", [])
