...
def _normalize_prerequisite_values(value: Any) -> List[str]:
    """Normalize prerequisite metadata without interpreting it as shell input."""
    if not value:
        return []
...
def _collect_prerequisite_commands(frontmatter: Dict[str, Any]) -> List[str]:
    prerequisites = frontmatter.get("prerequisites")
    if not isinstance(prerequisites, dict):
        return []
    return _normalize_prerequisite_values(prerequisites.get("commands"))
...
def _skill_readiness(frontmatter: Dict[str, Any], skill_name: str) -> Tuple[dict, dict]:
    ...
    required_commands = _collect_prerequisite_commands(frontmatter)
    ...
    setup_needed = bool(remaining or missing_required_commands)
    ...
    fields = {
        ...
        "missing_credential_files": missing_cred_files, "missing_required_commands": missing_required_commands,
