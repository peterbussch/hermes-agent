...
def test_run_doctor_accepts_vendor_slugs_for_user_gateway_provider(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text(
        "model:\n"
    ...
    out = buf.getvalue()
    assert "model.default 'codex/gpt-5.6-luna' uses a vendor/model slug" not in out
    assert "Either set model.provider to 'openrouter', or drop the vendor prefix." not in out
