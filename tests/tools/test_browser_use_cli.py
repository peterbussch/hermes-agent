...
class TestBrowserExec:
    def test_local_backend_readiness_failure_is_actionable(self, tmp_path, monkeypatch):
        ...
        # Exercise the no-CDP local path. Upstream now provisions managed
        # Chromium during routing, which is independently ready even when the
        # CLI's local-Chrome doctor probe fails.
        monkeypatch.setattr(bu_cli, "_route_backend", lambda *_args, **_kwargs: None)
        result = json.loads(bu_cli.browser_exec("print(1)"))
