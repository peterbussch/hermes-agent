...
def _apply_loopback_auth_override(kwargs: dict) -> dict:
    ...
    Local gateways like OmniRoute reject any non-empty "Bearer X" value (their
    REQUIRE_API_KEY=false setting only honors requests whose Authorization is
    missing or non-Bearer). When the resolved api_key is one of the placeholder
    strings used for "no auth needed" and the base_url is loopback, send an
    empty Authorization value via default_headers so the SDK's auto-generated
    "Bearer no-key-required" is overridden and the gateway's no-auth path
    applies. Empty value is used (not "Bearer ") because httpcore rejects
    trailing whitespace in header values with LocalProtocolError.
    """
